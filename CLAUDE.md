...
- `Pipeline.swift` — Orchestrates: WhisperKit transcribe → dictionary replacements → clipboard paste. The clipboard hold applies here (one-shot mode) only; live mode inserts via accessibility and must leave the clipboard alone. Two-phase warmUp: fast GPU load (~5-10s), then background ANE specialization.
