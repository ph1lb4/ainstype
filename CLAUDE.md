...
- `Pipeline.swift` — Orchestrates: WhisperKit transcribe → dictionary replacements → clipboard paste. The clipboard hold applies to the one paste here; in live mode `StatusMenuController` applies it to the finished transcript on release (never per chunk — that trampled the user's clipboard mid-dictation). Two-phase warmUp: fast GPU load (~5-10s), then background ANE specialization.
...
- `Clipboard.swift` — `NSPasteboard` + `CGEvent` Cmd+V paste (no osascript). Requires Accessibility permission. `RestoreLedger` handles restoring the user's clipboard after a synthetic paste; `copyAndHold` implements the `clipboard_hold_seconds` setting and `copyPinned` leaves text on the clipboard for good (cancelling any pending restore). `FocusSnapshot`/`PasteOutcome` verify delivery: a posted ⌘V returns nothing, so the focused element's character count and caret are compared before/after (never its text — privacy). `.failed` is deliberately conservative (no focused element, or a non-text target that didn't change) so editors that don't update those attributes don't produce false failures.
- `RecoveryBubble.swift` — Non-activating floating panel shown bottom-center when a transcription can't be delivered (paste/insert failed, transcription errored) or on "Copy Latest". Shows the text with a Copy button, auto-dismisses after 6s (hard cap 20s, hover suspends the countdown), never steals keyboard focus so ⌘V still goes to the user's app. Timers run in `.common` run loop modes so an open menu can't stall them. Replaces the previous `UNUserNotification` error path.
